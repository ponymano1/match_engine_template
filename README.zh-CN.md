...
```
  ...
  Redis Stream ─poll()──►│  RedisInbound.poll()                           │
                         ...
                         └───────────────┬───────────────┬───────────────┘
                                         │               │
                       每 symbol 一个 Sender<Sequenced>(有界输入环形)
                                         │               │
                          ┌──────────────▼───┐   ┌───────▼──────────┐
                          ...
                          └──────────────┬───┘   └───────┬──────────┘
                                         │               │
                  每 symbol 一个 Sender<Events>(有界输出环形,整批一次性发)
                                         │               │
                          ┌──────────────▼───┐   ┌───────▼──────────┐
                          │ publisher-BTC/USD │   │ publisher-ETH/USD│   ...
                          │ 独立 RedisOutbound │   │ 独立 RedisOutbound│
...
- **每个 symbol 一条独立链路**:独立订单簿 + 独立输入环 + 独立输出环 + 独立
  publisher 线程 + 独立 Redis 连接。某个 symbol 的下游(Redis)变慢或卡住,
  只会回压它自己那条链路,不会波及其它 symbol——故障隔离,爆炸半径锁在单 symbol。
- 撮合线程把 `handle` 返回的**整批 `Events` 一次性 send**(非逐事件发),
  output 侧 channel 操作降到 1 次/单;批内逐事件选 topic、序列化、publish 由
  对应 publisher 线程承担。
- **有界**通道在下游变慢时提供天然背压。
...
└─ 赋予 seq / shard_seq / ts
└─ tx.send(Sequenced)             ──────► (每 symbol 输入通道)
...
└─ out_tx.send(events)    ──────► (该 symbol 专属输出通道,整批一次)

publisher-<symbol> 线程
└─ out_rx.iter()                        // 收到整批 Events
└─ for ev in batch:
├─ 按 Trade / 其它 选 topic         // Trade → trades_topic,余 → book_topic
├─ serde_json::to_vec(&ev)
└─ RedisOutbound::publish(topic, payload)   // mq.rs,该 symbol 专属连接
...
main 循环 → 按 symbol 路由 → Sequenced → 每 symbol 输入通道
match-<symbol> 线程
└─ Engine::handle
└─ Command::Cancel
...
└─ out_tx.send(events)                    ──────► (输出路径同 NewOrder:
                                          整批 Events → 该 symbol 专属 publisher)
...
`output_ring_capacity` 是**每个 symbol 各自的**输出环容量(并非全局共享)。
...
- 每个 symbol 拆出独立 output 环 + 独立 publisher 线程 + 独立 Redis 连接,取代原先全局共享的单个 output 队列。收益是**故障隔离**:某 symbol 下游变慢只回压自己那条链路,不再连带阻塞其它 symbol。注意这是架构层面的隔离性改动,单 symbol 吞吐不受影响。(已完成)
